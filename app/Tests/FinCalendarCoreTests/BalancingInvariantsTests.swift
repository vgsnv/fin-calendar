...
/// соблюдаться (П7), а свободные деньги — не уходить в минус (П6а).
final class BalancingInvariantsTests: XCTestCase {
    ...
    func testMoneyAddsUpDatesHoldFreeMoneyStaysPositive() {
        ...
        let balancer = Balancer(namedWeek: 12_000)
        ...
        for _ in 0..<300 {
            ...
            // Финнедели — полные порции, статьи — со случайными сроками и суммами.
            ...
            let rec = balancer.recommend(incomes: incomes, weekStarts: weekStarts, needs: needs)

            // Деньги не появляются и не пропадают: приходы + недостачи = недели + статьи + свободные.
            let income = incomes.reduce(0) { $0 + $1.amount }
            let missing = rec.shortfalls.reduce(0) { $0 + $1.amount }
            let weeks = rec.weeks.reduce(0) { $0 + $1.amount }
            let demand = needs.reduce(0) { $0 + $1.amount }
            let free = rec.freeMoney.values.reduce(0, +)
            XCTAssertEqual(income + missing, weeks + demand + free, accuracy: 0.01)
            ...
            // свободных денег больше, значит переносить на него уже нечего — все взносы
            // бедного приход-богач не успевает по срокам (П7).
            for rich in incomes {
                for poor in incomes where poor.month == rich.month
                    ...
                    let movable = rec.contributions.filter {
                        $0.incomeId == poor.id && dueById[$0.needId]! >= rich.factDate
                    }.reduce(0) { $0 + $1.amount }
